...
export async function sendEmail(args: { to: string; subject: string; text: string }): Promise<DeliveryResult> {
...
export async function sendSms(args: { to: string; text: string }): Promise<DeliveryResult> {
  const sid = process.env.TWILIO_ACCOUNT_SID;
  const token = process.env.TWILIO_AUTH_TOKEN;
  const from = process.env.TWILIO_FROM_NUMBER;
  if (!sid || !token || !from) throw new Error("SMS provider is not configured");

  const form = new URLSearchParams({ To: args.to, From: from, Body: args.text });
  ...
  const body = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(body?.message || `SMS provider returned ${response.status}`);
