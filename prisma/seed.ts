...
if (process.env.ALLOW_DEMO_SEED !== "true") {
  throw new Error("Demo seed blocked. Set ALLOW_DEMO_SEED=true only for development/test databases.");
}

if (process.env.NODE_ENV === "production") {
  throw new Error("Demo seed must never run with NODE_ENV=production.");
}
...
async function main() {
  const school = await prisma.school.upsert({
    ...
    create: {
      ...
      settings: { create: { timezone: "Australia/Brisbane", currency: "AUD", preOrderEnabled: true, preOrderCutoffTime: "07:00", allowNegativeBalance: true, minimumAllowedBalance: -10 } },
      pickupSlots: { create: [
  ...
  if (superAdminEmail && superAdminPassword) {
    ...
    await prisma.user.upsert({ where: { email: superAdminEmail }, update: { fullName: "CanteenCo Super Admin", passwordHash, role: UserRole.SUPER_ADMIN, status: UserStatus.ACTIVE }, create: { fullName: "CanteenCo Super Admin", email: superAdminEmail, passwordHash, role: UserRole.SUPER_ADMIN, status: UserStatus.ACTIVE } });
  }
  ...
  if (cashierEmail && cashierPassword) {
    ...
    await prisma.user.upsert({ where: { email: cashierEmail }, update: { fullName: "Demo Cashier", passwordHash, role: UserRole.CASHIER, status: UserStatus.ACTIVE, schoolId: school.id }, create: { fullName: "Demo Cashier", email: cashierEmail, passwordHash, role: UserRole.CASHIER, status: UserStatus.ACTIVE, schoolId: school.id } });
  }
